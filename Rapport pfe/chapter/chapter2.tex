...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.80\textwidth]{diagrams/usecase.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{diagrams/platform_usecase.png}
...
\subsubsection{Administrative email confirmation sequence}
Figure \ref{fig:sequence_admin_confirm} shows the process of manual email confirmation by an administrator. It outlines how the manager selects a user on the accounts management panel, triggers the confirmation service, and calls the database RPC to directly bypass standard verification checks.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{diagrams/admin_confirm_diagram.png}
    \caption{Sequence Diagram - Administrative Email Confirmation}
    \label{fig:sequence_admin_confirm}
\end{figure}
...
The use case design maps objects to the classic class diagram. Figure \ref{fig:class_diag_classic} shows the classic class diagram including the attributes, methods, and relationships of the system's main entities.
\begin{figure}[H]
    \centering
    \includegraphics[width=1.01\textwidth]{diagrams/class_diag_classic.png}
    \caption{Classic Class Diagram of the System}
    \label{fig:class_diag_classic}
\end{figure}

The use case design also maps objects to the logical class diagram. Figure \ref{fig:class_diag} shows the model class relationships matching the database schema.
\begin{figure}[H]
    \centering
...
\begin{itemize}
    ...
    \item \textbf{Platform User Verification Flag:} The \textit{PlatformUser} class includes an \texttt{emailConfirme} boolean attribute. This property represents the verification state of administrative staff members, allowing administrators to manually verify and bypass standard authentication gates directly within the accounts registry.
\end{itemize}
...
\hline
Version Control & Git \& GitHub \cite{GitHub} & Source code versioning and remote repository hosting \\
\hline
...
The codebase of both the mobile and web client applications is structured around a decoupled \textbf{Model-View-Controller (MVC)} design pattern \cite{Gamma1994} to separate business logic from the user interface presentation:
...
\\
\textbf{Web Application} : Configured for local execution during development and deployed on the cloud via \textbf{Vercel} \cite{VercelDocs} for production, ensuring global accessibility for administrators.
\\
\textbf{Mobile Client App} : Compiled and deployed as an Android package (APK) for mobile client managers.
\\
\textbf{Database \& API} : Hosted on the cloud using the managed \textbf{Supabase} \cite{SupabaseDocs} backend platform (incorporating the PostgreSQL database, cloud storage, and real-time engine).
