...
This chapter introduces the graduation project and its general context. It first presents the internship environment and the organizational structure of the host company, HMI Stars Consulting. It then outlines the main problem related to manual administrative workflows and introduces the proposed digital solution. Finally, it compares existing ERP solutions such as Odoo, ERPNext, and Dolibarr to support the choice of technologies and system architecture.
...
\begin{enumerate}
    ...
    \item \textbf{A Real-Time Backend Server:} Directly solves the lack of real-time synchronization and instant feedback identified in both ERPNext and Dolibarr. Powered by Supabase, it manages instant messaging and file sharing via WebSocket streams, while ensuring absolute data isolation between client companies using PostgreSQL Row Level Security (RLS) policies.
\end{enumerate}
