...
\subsubsection{Leave Request Submission \& Approval Sequence}
Figure \ref{fig:sequence_leave_design} shows the lifecycle of a leave request, highlighting how the Client submits it, how Row Level Security policies validate the transaction, and how notifications are managed.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{diagrams/leave_sequence_diagram.png}
    \caption{Sequence Diagram - Leave Request \& Processing Flow}
...
The logical class models are mapped to physical PostgreSQL tables in the Supabase database schema:
\\
...
\textbf{3. Client Management Subsystem :} Employees list, attendance pointage forms, and leave requests.\\
...
\textbf{web application : } Deployed on Vercel hosting.\\
\textbf{Mobile Client App : }Compiled as an Android APK.\\
\textbf{Database and API : } Hosted on Supabase cloud.

\subsection{Use Case Implementation}
We implemented the features iteratively, starting with model classes, then integrating database API calls, and finally completing the user interface components.

\section{Conclusion}
This chapter detailed the design development using the Unified Process structure. The next chapter will focus on theTransition phase, functional testing, and user evaluation of the implemented system.
